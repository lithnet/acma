...
namespace Lithnet.Acma
{
    ...
    public class MADataContext : IDisposable
    {
        ...
        private Dictionary<Guid, MAObjectHologram> hologramCache = new Dictionary<Guid, MAObjectHologram>();
        ...
        public MADataContext(SqlConnection connection)
        {
            this.dbc = new DBConnection(connection);
        }
        ...
        public MAObjectHologram GetMAObjectOrDefault(Guid objectId, AcmaSchemaObjectClass objectClass)
        {
            ...
            command.Connection = this.dbc.Connection;
        ...
        /// <summary>
        /// Determines if an attribute name and value pair exist in the database
        /// </summary>
        /// <param name="attributeName">The name of the attribute</param>
        /// <param name="attributeValue">The attribute value</param>
        /// <returns>A value indicating whether the attribute and value exists on the object</returns>
        public bool DoesAttributeValueExist(string attributeName, string attributeValue)
        {
            AcmaSchemaAttribute attribute = ActiveConfig.DB.GetAttribute(attributeName);
            return this.DoesAttributeValueExist(attribute, attributeValue);
        }
        ...
        /// <param name="attributeValue">The attribute value</param>
        /// <returns>A value indicating whether the attribute and value exists on the object</returns>
        public bool DoesAttributeValueExist(AcmaSchemaAttribute attribute, object attributeValue)
        {
            SqlCommand command = new SqlCommand();
            command.Connection = this.dbc.Connection;
            command.CommandType = CommandType.Text;
            ...
            {
                command.CommandText = string.Format("SELECT 1 FROM [dbo].[{0}] WHERE [AttributeName] = '{1}' AND [{2}] = @value", attribute.TableName, attribute.Name, attribute.ColumnName);
            }
            else
            {
                command.CommandText = string.Format("SELECT 1 FROM [dbo].[{0}] WHERE [{1}] = @value", attribute.TableName, attribute.ColumnName);
            }
