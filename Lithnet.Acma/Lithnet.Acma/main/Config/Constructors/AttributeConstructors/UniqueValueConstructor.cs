...
namespace Lithnet.Acma
{
    ...
    public class UniqueValueConstructor : AttributeConstructor
    {
        ...
        internal override void Execute(MAObjectHologram hologram)
        {
            ...
            {
                throw new ConfigurationException("There were no attributes to process unique allocations against");
            }
            ...
            {
                returnValues = this.ValueDeclaration.Expand(hologram, (string t) => this.IsAttributeUnique(hologram, t));
            }
            else
            {
        ...
        protected override IEnumerable<SchemaAttributeUsage> GetAttributeUsageInternal(string parentPath, AcmaSchemaAttribute attribute)
        {
            ...
            {
                yield return new SchemaAttributeUsage(this.GetType().Name, parentPath, "Unique allocation attribute");
            }
        ...
        private bool IsAttributeUnique(MAObjectHologram hologram, string valueToTest)
        {
            ...
            {
                DBQueryByValue query = new DBQueryByValue(attribute, ValueOperator.Equals, valueToTest);
                group.AddChildQueryObjects(query);
                //if (hologram.MADataContext.DoesAttributeValueExist(attribute, valueToTest))
                //{
                ///   return false;
                //}
            }

            return !hologram.MADataContext.GetMAObjectsFromDBQuery(group).Any(t => t.Id != hologram.Id);
        }
    }
}
