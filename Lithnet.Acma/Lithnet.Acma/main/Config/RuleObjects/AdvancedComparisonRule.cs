...
namespace Lithnet.Acma
{
    ...
    public class AdvancedComparisonRule : Rule
    {
        ...
        public override IEnumerable<SchemaAttributeUsage> GetAttributeUsage(string parentPath, AcmaSchemaAttribute attribute)
        {
            ...
            {
                yield return new SchemaAttributeUsage("Advanced value comparison rule",
                   parentPath,
            ...
            {
                yield return new SchemaAttributeUsage("Value comparison rule",
                   parentPath,
                   string.Format("{0} {1} {2}", this.SourceValue.Declaration, this.ValueOperator, this.TargetValue.Declaration));
            }
        }
        ...
        private bool EvaluateNew(MAObjectHologram sourceObject)
        {
            ...
            if (sourceValues.Count == 0)
            {
                ...
                {
                    if (this.ValueOperator == Fim.Core.ValueOperator.NotEquals)
                    {
                        return true;
                    }
                    else
            ...
            if (this.ValueOperator == Fim.Core.ValueOperator.IsPresent)
            {
            ...
            if (this.ValueOperator == Fim.Core.ValueOperator.NotPresent)
            {
        ...
        protected bool EvaluateAttributeValue(object actualValue, IList<object> expectedValues)
        {
            ...
            if (!result)
            {
                this.RaiseRuleFailure("Comparison failed\nComparison Operator: {1}\nExpected Values: {2}\nActual Value: {3}", this.ValueOperator.ToString(), expectedValues.Select(t => t.ToSmartStringOrNull()).ToCommaSeparatedString(), actualValue.ToSmartString());
            }
        ...
        private void Initialize()
        {
            ...
            this.ValueOperator = Fim.Core.ValueOperator.Equals;
        }
