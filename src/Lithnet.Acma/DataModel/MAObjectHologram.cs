    ...
    public class MAObjectHologram : MAObject, ISerializable
    {
        ...
        public MAObjectHologram ShadowParent
        {
            get
            {
                if (this.shadowParent == null)
                {
                    ...
                    this.shadowParent = ActiveConfig.DB.GetMAObjectOrDefault(this.ShadowParentID, this.ShadowLink.ParentObjectClass);
                }
            ...
            foreach (AcmaSchemaShadowObjectLink link in this.ObjectClass.ShadowChildLinks)
            {
                ...
                else
                {
                    ...
                    if (provisioningControlValue.ValueBoolean == true)
                    {
                        ...
                        MAObjectHologram shadowChild = ActiveConfig.DB.GetMAObjectOrDefault(referenceValue.ValueGuid, link.ShadowObjectClass);
        ...
        internal MAObjectHologram ProvisionShadowObject(AcmaSchemaShadowObjectLink shadowLink, CSEntryChange csentry = null)
        {
            ...
            if (shadowObject == null)
            {
                ...
                {
                    newId = new Guid(csentry.DN);
                }

                shadowObject = ActiveConfig.DB.CreateMAObject(newId, shadowLink.ShadowObjectClass.Name, this, ObjectModificationType.Add);
        ...
        internal void DeprovisionShadowObject(AcmaSchemaShadowObjectLink shadowLink)
        {
            ...
            {
                MAObjectHologram shadowObject = ActiveConfig.DB.GetMAObjectOrDefault(shadowObjectId.ValueGuid, shadowLink.ShadowObjectClass);
                ...
                if (shadowObject != null)
                {
                    ...
                    //this.DBDeleteAttribute(shadowLink.ReferenceAttribute);
        ...
        private MAObjectHologram UndeleteShadowObject(AcmaSchemaShadowObjectLink link, Guid existingId)
        {
            ...
            MAObjectHologram shadowObject = ActiveConfig.DB.GetMAObjectOrDefault(existingId, link.ShadowObjectClass);
        ...
        private void DeleteShadowObjects()
        {
            ...
            List<MAObjectHologram> shadowObjects = ActiveConfig.DB.GetMAObjectsFromDBQuery(ActiveConfig.DB.GetAttribute("shadowParent"), ValueOperator.Equals, this.ObjectID).ToList();
        ...
        internal void Delete(bool dereferenceShadowParent)
        {
            ...
            {
                ActiveConfig.DB.DeleteMAObjectPermanent(this.ObjectID);
            }
        ...
        private void DereferenceObject(int depth = 0)
        {
            ...
            Dictionary<Guid, IList<string>> referencingObjects = ActiveConfig.DB.GetReferencingObjects(this);

            foreach (KeyValuePair<Guid, IList<string>> kvp in referencingObjects)
            {
                MAObjectHologram referencingObject = ActiveConfig.DB.GetMAObjectOrDefault(kvp.Key);
            ...
            foreach (AttributeChange attributeChange in this.InternalAttributeChanges)
            {
                    ...
                    t.InheritedAttribute.Name == attributeChange.Name))
                {
                    ...
                    if (!queryResultCache.ContainsKey(key))
                    {
                        ...
                        queryResultCache.Add(key, ActiveConfig.DB.GetMAObjectsFromDBQuery(queryGroup).ToList());
                    }
        ...
        private IEnumerable<MAObjectHologram> GetReferencedObjectsProposed(AcmaSchemaAttribute attribute)
        {
            ...
            {
                MAObjectHologram referencedObject = ActiveConfig.DB.GetMAObjectOrDefault(value.ValueGuid);
